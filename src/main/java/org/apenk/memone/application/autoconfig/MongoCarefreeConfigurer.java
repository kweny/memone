...
public class MongoCarefreeConfigurer implements ImportBeanDefinitionRegistrar, EnvironmentAware, ApplicationContextAware {
    ...
    private MongoCarefreeOption createMongoCarefreeOption(MongoCarefreeArchetype archetype) {
        MongoCarefreeOption option = new MongoCarefreeOption();

        List<MongoCarefreeOptionedListener> optionedListeners = archetype.getOptionedListeners();
        if (optionedListeners != null && optionedListeners.size() > 0) {
            for (MongoCarefreeOptionedListener optionedListener : optionedListeners) {
                optionedListener.optionCreated(option);
            }
        }
